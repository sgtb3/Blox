...
let translate (globals,functions) =
  ...
  (* print_endline "translate called\n"; *)
  
  let context  = L.global_context () in
  let blox_mod = L.create_module context "blox"
  ...
  let function_decls =
    let function_decl map fdecl =
      ...
      in 
      let ftype = 
        L.function_type (ltype_of_typ fdecl.A.typ) formal_types 
      in
    ...
    let local_vars =
      ...
      let local_bindings = 
        let rec make_type_id_mapping = function
          | []       -> []
          | hd :: tl -> (match hd with 
                          | A.Var_Decl(dt,id)    -> [(dt,id)]
                          | A.Var_Assign(va,exp) -> [(fst va,snd va)]
                          | _                    -> []) 
                         @ make_type_id_mapping (List.filter ((<>) hd) tl)
        in 
